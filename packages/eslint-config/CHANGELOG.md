# @takanome-dev/eslint-config

## 1.1.0

### Minor Changes

- 87f2545: add tailwindcss eslint & prettier configs

### Patch Changes

- Updated dependencies [87f2545]
  - @takanome/eslint-config-typescript@1.0.7

## 1.0.6

### Patch Changes

- update package scopes
- Updated dependencies
  - @takanome/eslint-config-typescript@1.0.6

## 1.0.5

### Patch Changes

- change package org name and publish back to npm
- Updated dependencies
  - @takanome/eslint-config-typescript@1.0.5

## 1.0.4

### Patch Changes

- remove jsdoc plugin from eslint base configs
- Updated dependencies
  - @takanome-dev/eslint-config-typescript@1.0.4

## 1.0.3

### Patch Changes

- update packages namespace
- Updated dependencies
  - @takanome-dev/eslint-config-typescript@1.0.3

## 1.0.2

### Patch Changes

- Add airbnb and airbnb hooks configs in my eslint config
- Updated dependencies
  - @takanome/eslint-config-typescript@1.0.2

## 1.0.1

### Patch Changes

- @takanome/eslint-config-typescript@1.0.1

## 1.0.0

### Major Changes

- d8c0194: Add documentation on how to install and use the packages

### Patch Changes

- Updated dependencies [d8c0194]
  - @takanome/eslint-config-typescript@1.0.0

## 1.0.0

### Major Changes

- Add documentation on how to install and use the packages

### Patch Changes

- Updated dependencies
  - @takanome/eslint-config-typescript@1.0.0

## 0.3.1

### Patch Changes

- Fix package dependency mismatch

## 0.3.0

### Minor Changes

- Update package json configs and move from ts to js

### Patch Changes

- Updated dependencies
  - @takanome/eslint-config-base@0.3.0

## 0.2.0

### Minor Changes

- Fix `ERR_PNPM_WORKSPACE_PKG_NOT_FOUND` error when trying to install `@takanome/eslint-config-typescript` in a project.

### Patch Changes

- Updated dependencies
  - @takanome/eslint-config-base@0.2.0
